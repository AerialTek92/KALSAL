...
class SemiFinishedQCInherit(models.Model):
    ...
    # ==========================================
    # DROPDOWN FILTER: delegate to the base quantity-aware logic
    # ==========================================
    @api.depends('sale_order_id')
    def _compute_allowed_recipe_product_ids(self):
        """The base module (fk_semi_finished_qc) now owns eligibility:
        mixing done + Line Clearance confirmed + available qty > 0 + no open QC.
        The rework lifecycle is enforced through _get_consumed_qtys below —
        the old existence-based filtering here was hiding partially-QC'd
        products and must NOT be re-applied."""
        for rec in self:
    ...
    # ==========================================
    # REWORK-AWARE QUANTITY RELEASE
    # ==========================================
    def _get_consumed_qtys(self, so, product, exclude_id=False):
        """Same as base, BUT a failed QC whose rework is COMPLETED releases
        its quantity back to the pool — the post-rework QC will consume it.
        (Failed with no rework, or rework still pending = qty stays consumed,
        so the product stays hidden until the rework is done.)"""
        passed, failed = super()._get_consumed_qtys(so, product, exclude_id=exclude_id)
        ...
        failed_qcs = self.search(domain)

        released = 0.0
        for qc in failed_qcs:
            sheet = self.env['kalsal.rework.sheet'].search([
                ('semi_finished_qc_id', '=', qc.id)], limit=1)
            if sheet and (sheet.state == 'passed' or sheet.post_rework_qc_id):
                released += qc.qty_to_qc
        return passed, failed - released

    # ==========================================
    # LEGACY HELPER (kept for reference; no longer called by the dropdown)
    # ==========================================
