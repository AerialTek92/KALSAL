...
class KalsalQualityCheck(models.Model):
    ...
    # NEW: Consolidated Live Notification Helper (Inbox Panel + Toast Popup)
    def _send_live_notification(self, title, plain_message, sticky=False, warning=True):
        if not self.responsible_id or not self.responsible_id.partner_id:
            return

        partner = self.responsible_id.partner_id
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')

        # Link directly to this specific Quality Check form
        url = f"{base_url}/web#id={self.id}&model={self._name}&view_type=form"

        # 1. Inbox Panel Notification (Secured HTML)
        notify_ctx = dict(self.env.context, mail_notify_force_send=False)
        inbox_body = Markup(
            _("%s <br/><br/><a href='%s' target='_blank'><b>Click here</b></a> to open the Quality Check record.")) % (
                         plain_message, url)
        ...
        # 2. Toast Popup Notification (Top Right Corner)
        notification_vals = {
            'type': 'warning' if warning else 'info',
            'title': title,
            'message': plain_message,
            'sticky': sticky,
        }
        self.env['bus.bus']._sendone(partner, 'simple_notification', notification_vals)
    ...
    def _send_delay_notification(self):
        ...
        msg = _(
            "The %s-day delay period for Quality Check %s has ended.\n\nDelayed parameters requiring results:\n%s\n\nPlease proceed with the 2nd QC to complete these tests.") % (
                  self.delay_days, self.name, delayed_params_text)

        # Send Live Notification (Inbox + Popup) replacing Activities
        self._send_live_notification(
            title=_('Delay Period Ended - 2nd QC Required'),
            plain_message=msg,
            sticky=True,
            warning=True
        )
        ...
        if not self.delay_notification_sent:
            # Leave note in the record's chatter history
            self.message_post(
                body=Markup(_(
    ...
    def _send_delay_reminder(self):
        ...
        # Build plain text message
        due_str = self.delay_due_date.strftime('%Y-%m-%d %H:%M') if self.delay_due_date else 'N/A'
        msg = _(
            "The delay period for Quality Check %s will end on %s. Please prepare to enter results for the delayed parameters.") % (
                  self.name, due_str)

        # Send Live Notification (Inbox + Popup) replacing Activities
        self._send_live_notification(
            title=_('Delay Period Ending Soon'),
            plain_message=msg,
            sticky=False,
            warning=False
        )
