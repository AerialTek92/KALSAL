...
# ==========================================
# EXTEND THE FG SPEC LINE (product tab) FROM THIS MODULE
# ==========================================
class FinishedQualityParameterLineInherit(models.Model):
    _inherit = 'finished.quality.parameter.line'  # ← the REAL model behind fg_specs
    ...
    is_microbiological = fields.Boolean(
        ...
        help="Tick to route this parameter into the Microbiological Tests tab "
             "of the Finished Goods QC. Unticked parameters stay in the "
             "standard Test Parameters tab.")

    condition = fields.Selection(
        selection_add=[('detection', 'Detection Test (Present / Absent)')])
...
class FinishedQC(models.Model):
    ...
    def action_pass(self):
        ...
        # 2. GATE: Every detection test must have an explicit choice
        unselected = all_lines.filtered(
        ...
        # 3. GATE: "Present" detection tests require remarks to pass
        present_lines = all_lines.filtered(
            lambda l: l.condition == 'detection' and l.detection_result == 'present')
    ...
    def action_reload_default_parameters(self):
        ...
        for rec in self:
            ...
            (rec.standard_line_ids | rec.microbiological_line_ids).unlink()
