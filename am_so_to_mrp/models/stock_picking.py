...
class StockLot(models.Model):
    ...
    check_location_id = fields.Many2one('stock.location', 'Location', domain="[('usage', '!=', 'view')]")

    # @api.depends('quant_ids', 'quant_ids.quantity', 'check_location_id')
    # def _compute_single_location(self):
    #     for lot in self:
    #         print('lot.id', lot.id)
    #         print('lot.check_location_id', lot.check_location_id)
    #         # lot.check_location_id = self.check_location_id
    #         # if lot.check_location_id:
    #         #     lot.location_id = lot.check_location_id.id
    #         # else:
    #         quants = lot.quant_ids.filtered(lambda q: q.quantity >= 1)
    #         lot.location_id = quants.location_id if len(quants.location_id) == 1 else False
    #         print('lot.location_id', lot.location_id)
    #         print('lot.location_id', lot.location_id)
            ...
            if picking.picking_type_id.code == "incoming":
                for move_line in picking.move_ids:
                    ...
                    while True:
                        ...
                        print('existing_lot', existing_lot)
                        if not existing_lot:
                            lot_name = temp_name
                            break
